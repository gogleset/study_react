...
// import React, {useState} from "react"; 이런 식으로도 가능함

const MyState = () => {
  /**
   * state(상태)값 정의 (프로그램이 실행되는 동안 지속된다. 즉, 컴포넌트가 화면에 표시될 때 지속되는 변수)
   * - 이 페이지 안에서 유효한 전역변수 같은 개념
   * - const [변수이름, 변수에 대한 setter 함수] = React.useState(변수의 기본값);
   * - state값은 직접 변경할 수 없고 반드시 setter를 통해서만 변경 가능하다.
   * - useState() 함수에 전달하는 값은 state값에 대한 초기 값이다.
   */
  ...
  const onMyNameChange = (e) => {
    // e.currentTarget 은 jQuery의 $(this)에 해당함.
