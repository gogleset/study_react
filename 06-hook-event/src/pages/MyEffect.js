...
const MyEffect = () => {
    ...
    */
    ...
    /**실행 순서1. 이 컴포넌트가 화면에 막 등장할 때와 state, props값이 변경될 때마다 매번 실행됨 
     * 특정 값을 넣은게 아니기때문에
    */
    React.useEffect(() => {
    ...
    /**실행 순서2. 이 컴포넌트가 화면에 막 등장할 때와 특정 state, props값이 변경될 때만 실행됨 
     * 배열을 넣어주면 특정 값만 바꾸라라는 것, 어떠한 값을 모니터링하는 것처럼 동작하게 된다.
    */
    React.useEffect(() => {
